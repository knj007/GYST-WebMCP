...
- The application has a marketing page, sign-in flow, Supabase SSR clients, centralized `getClaims()` authorization, and request-dynamic authenticated daily/weekly route shells. The current local Wave 3 branch adds the ordinary daily draft form and a provisional commit action; security review has blocked its human-only authorization boundary pending an owner-approved redesign. Weekly ritual, WebMCP tools, reminders, and demo ledger remain unfinished.
...
- Production contains only the two tracked Wave 2 migrations: `20260830160046_remediate_untracked_rls_helper.sql` and `20260830194920_wave2_application_schema.sql`. The current local Wave 3 branch adds unreviewed `20260830211216_daily_ritual_commit.sql`; it is not a remote-migration candidate without a new A8 evidence gate.
- The untracked `ensure_rls` event trigger and `public.rls_auto_enable()` function are absent locally and remotely. The Wave 2 production gate passed at 120 pgTAP assertions. The local Wave 3 reset passes 135 diagnostic assertions, but reviewer analysis showed its transaction-local RLS marker is forgeable by a SQL-capable authenticated caller; it does not meet the human-only boundary and W3 is not passed. Production remains at the verified 11 RLS-enabled application tables, zero `anon` application-table grants, and 38 operation-specific `authenticated` policies.
...
Current implementation status: the local prototype's `SECURITY INVOKER` RPC uses a transaction-local PostgreSQL setting to distinguish its final session update from ordinary updates. That setting is mutable by a SQL-capable authenticated caller, so it is not an authorization capability. Do not apply or promote this migration. The owner must choose either a separate trusted server-side capability (which requires a new secret/configuration approval and a new security design review) or an explicitly narrower application-only guarantee. The existing prohibition on `SECURITY DEFINER` permission workarounds remains in force.
