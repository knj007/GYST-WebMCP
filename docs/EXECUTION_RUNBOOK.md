...
- The untracked `ensure_rls` event trigger and `public.rls_auto_enable()` function are absent locally and remotely. The Wave 2 production gate passed at 120 pgTAP assertions. The local Wave 3 reset passes 156 assertions, including atomic draft save/rollback, stale-version rejection, validated daily close, cross-owner denial, and idempotent replay. It is not yet W3-complete: authenticated browser E2E, the final WebMCP negative-tool contract, and independent review remain. Production remains at the verified 11 RLS-enabled application tables, zero `anon` application-table grants, and 38 operation-specific `authenticated` policies.
