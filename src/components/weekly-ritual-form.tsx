...
export function WeeklyRitualForm({ ritual }: { ritual: WeeklyRitual }) {
  ...
  return <form className="mt-8 space-y-6" action={saveAction}>
    ...
    <fieldset disabled={pending} className="space-y-6">
      <label className="block text-sm font-semibold" htmlFor="missing_metrics">Missing metrics</label>
      <textarea className="mt-2 min-h-20 w-full rounded-xl border border-line bg-background p-3" defaultValue={toLines(entry?.missing_metrics)} id="missing_metrics" name="missing_metrics" placeholder="One missing metric per line" />
      <label className="block text-sm font-semibold" htmlFor="observations">Observations</label>
      <textarea className="mt-2 min-h-28 w-full rounded-xl border border-line bg-background p-3" defaultValue={toLines(entry?.observations)} id="observations" name="observations" placeholder="What did the week show you? One observation per line." />
      <label className="block text-sm font-semibold" htmlFor="decision_text">Decision</label>
      <textarea className="mt-2 min-h-24 w-full rounded-xl border border-line bg-background p-3" defaultValue={entry?.decision_text ?? ""} id="decision_text" maxLength={12000} name="decision_text" required />
      <label className="block text-sm font-semibold" htmlFor="arrow">Arrow</label>
      <select className="mt-2 w-full rounded-xl border border-line bg-background p-3" defaultValue={entry?.arrow ?? ""} id="arrow" name="arrow" required><option value="">Choose an arrow</option><option value="up">Up</option><option value="steady">Steady</option><option value="down">Down</option></select>
      <label className="block text-sm font-semibold" htmlFor="priorities">Dated priorities</label>
      <textarea className="mt-2 min-h-24 w-full rounded-xl border border-line bg-background p-3" defaultValue={priorityLines(entry?.priorities)} id="priorities" name="priorities" placeholder={'Priority title | 2026-09-07\nSecond priority | 2026-09-09'} required />
    </fieldset>
