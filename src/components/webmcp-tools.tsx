...
const dailyTools: Tool[] = [
  ...
  mutation("gyst.record_blocker", "Record or clear the blocker in the current daily draft.", { text: { type: "string", maxLength: 8000 }, type: { type: "string", enum: ["internal", "external_gate", "capacity", "clarity", "dependency", "other"] } }),
...
function mutation(name: string, description: string, properties: Record<string, object>, required: string[] = []): Tool {
  ...
  return { name, description, inputSchema: { type: "object", additionalProperties: false, properties, required }, execute: async (input) => {
...
export function WebMcpTools({ ritual }: { ritual: "daily" | "weekly" }) {
  ...
  useEffect(() => { const listener = (event: Event) => setChanges((current) => [String((event as CustomEvent<string>).detail), ...current].slice(0, 5)); window.addEventListener("gyst:webmcp-draft-updated", listener); return () => window.removeEventListener("gyst:webmcp-draft-updated", listener); }, []);
  useEffect(() => {
