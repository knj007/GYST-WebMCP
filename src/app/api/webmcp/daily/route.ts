...
export async function POST(request: Request) {
  try {
    ...
    switch (action) {
      case "record_moved": form.set("moved_text", requiredText(values.text, 12000, "text")); break;
      case "record_blocker": {
      ...
      case "score_previous_commitment": form.set("previous_commitment_id", requiredUuid(values.commitment_id, "commitment_id")); form.set("previous_commitment_outcome", enumValue(values.outcome, commitmentOutcomes, "outcome")); break;
      case "set_next_commitment": form.set("next_commitment_id", requiredUuid(values.commitment_id, "commitment_id")); break;
      case "record_optional_context": {
        for (const [key, limit] of [["buried_win", 4000], ["optional_context", 12000]] as const) { const text = optionalText(values[key], limit, key); if (text) form.set(key, text); else form.delete(key); }
    ...
    return NextResponse.json({ message: "Draft updated. It was not committed.", period_start: ritual.periodStart });
  } catch (error) {
